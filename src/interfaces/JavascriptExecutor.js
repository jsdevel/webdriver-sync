...
function coerce(result){
  ...
  if(java.instanceOf(result, classPaths.WebElement))return new WebElement(result);
  if(java.instanceOf(result, classPaths.Long))return result.longValue;
  if(java.instanceOf(result, classPaths.List)){
    return toArray(result, function(item){
      ...
      if(isWebElementArray)return new WebElement(item);
      return item;
    });
  }
}
