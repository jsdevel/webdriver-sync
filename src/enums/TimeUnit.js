...
TimeUnit.DAYS         = TimeUnit.prototype.DAYS         = new TimeUnit(Class.DAYS);
TimeUnit.HOURS        = TimeUnit.prototype.HOURS        = new TimeUnit(Class.HOURS);
TimeUnit.MICROSECONDS = TimeUnit.prototype.MICROSECONDS = new TimeUnit(Class.MICROSECONDS);
TimeUnit.MILLISECONDS = TimeUnit.prototype.MILLISECONDS = new TimeUnit(Class.MILLISECONDS);
TimeUnit.MINUTES      = TimeUnit.prototype.MINUTES      = new TimeUnit(Class.MINUTES);
TimeUnit.NANOSECONDS  = TimeUnit.prototype.NANOSECONDS  = new TimeUnit(Class.NANOSECONDS);
TimeUnit.SECONDS      = TimeUnit.prototype.SECONDS      = new TimeUnit(Class.SECONDS);

TimeUnit.prototype.convert=function(duration, unit){
   assert(duration).isNumber().throws("The first argument must be a number.");
   assert(unit).isInstanceof(TimeUnit).throws("The second argument must be a TimeUnit.");
...
TimeUnit.valueOf=TimeUnit.prototype.values=function(){
   return this._instance.valuesSync().map(function(v){return new TimeUnit(v);});
};